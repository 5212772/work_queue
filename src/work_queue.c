...
typedef struct tag_WORK_TASK_S {
    ...
    long                run_time;          /* This run time.              Unit:ms */
...
static int check_wq_id(int wq_id)
{
    ...
    if (NULL == g_wq_array[wq_id] || g_wq_array[wq_id].status) {
...
static int init_work_queue(void)
{
    ...
    //memset(g_wq_id_flag, 0, sizeof(int) * MAX_WORK_QUEUE_NUM);
...
int create_work_queue(int poll_time, int *wq_id)
{
    ...
    pthread_attr_setscope(&attr, PTHREAD_SCOPE_SYSTEM);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    ret = pthread_create(&g_wq_array[wq_id], &attr, work_queue_proccess, NULL);
    usleep(10);
    pthread_attr_destroy(&attr);
    if (ret) {
        STA_ERR_PRT ("Create pthread fail! ret:%d  error:%s\n", ret, strerror(errno));
...
int start_work_queue(int wq_id)
{
    ...
    if (NULL == g_wq_array[wq_id] || 0 == g_wq_id_flag[wq_id]) {
        WQ_ERR("The id:%d work_queue don't init! \n", wq_id);
        return -1;
    }
